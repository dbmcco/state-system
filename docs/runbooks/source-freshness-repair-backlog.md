...
| LFW processed transcripts missing/planned | LFW | Processed transcript understanding remains unavailable to Caroline. | Build the processed transcript pipeline, declare pipeline dependency from raw transcripts, emit index manifest and freshness watermark. | Processed transcript source has access passed, freshness fresh or bounded stale, index ready, and route/package refs include transcript evidence only when usable. |
| Navicyte scaffold connectors unproved | Navicyte | Helena scaffold declares modules but should treat them as not yet production-ready until preflight/freshness/index records exist. | Run source-owned preflight and freshness checks for declared modules, then generate package/read model with typed gap refs. | Every declared Navicyte module has explicit access, freshness, index status, checked_at, and gap refs where not ready. |
| Synthyra scaffold connectors unproved | Synthyra | Ingrid scaffold declares modules but should treat them as not yet production-ready until preflight/freshness/index records exist. | Run source-owned preflight and freshness checks for declared modules, then generate package/read model with typed gap refs. | Every declared Synthyra module has explicit access, freshness, index status, checked_at, and gap refs where not ready. |
