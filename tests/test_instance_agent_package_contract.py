...
class InstanceAgentPackageContractTests(unittest.TestCase):
    ...
    def test_personal_package_keeps_spotify_freshness_gap_visible(self):
        package = load_json(
            EXAMPLE_DIR / "instance-agent-package-braydon-personal-samantha.json"
        )
    ...
    def test_lfw_package_excludes_personal_sources(self):
        package = load_json(EXAMPLE_DIR / "instance-agent-package-lfw-caroline.json")
        encoded = str(package)
    ...
    def test_lfw_package_includes_governed_relationship_index_route_without_personal_materialization(self):
        package = load_json(EXAMPLE_DIR / "instance-agent-package-lfw-caroline.json")
        route = _route(package, "question_route.lfw.federated_relationship_index")
