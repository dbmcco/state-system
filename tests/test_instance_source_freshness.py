...
EXAMPLE = (
    ...
    / "instance-source-freshness-braydon-personal-msgvault.json"
)
...
class InstanceSourceFreshnessTests(unittest.TestCase):
    ...
    def test_record_persists_freshness_without_proving_access_or_authority(self):
        with TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                "state_instance.braydon_personal|connector.personal.msgvault|"
                "msgvault:tenant:personal-email",
                record["scope_key"],
            )
            self.assertTrue(
                record["id"].startswith(
                    "instance_source_freshness.state_instance.braydon_personal"
                )
            )
