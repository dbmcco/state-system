...
class QuestionRouteContractTests(unittest.TestCase):
    ...
    def test_personal_relationship_route_structures_no_calendar_only_rule(self):
        ...
        self.assertIn("connector.personal.lfw_state_system", route["source_order"])
    ...
    def test_lfw_federated_relationship_route_preserves_no_materialization_boundary(self):
        routes = _routes_by_id(load_json(REGISTRY))
        route = routes["question_route.lfw.federated_relationship_index"]
        ...
        self.assertIn("gap.state_instance.lfw.connector.lfw.linear.freshness_failed", route["gap_behavior"]["relevant_gap_refs"])

    def test_small_consulting_route_keeps_workboard_and_lfw_context_optional(self):
        routes = _routes_by_id(load_json(REGISTRY))
