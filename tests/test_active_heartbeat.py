...
class ActiveHeartbeatTests(unittest.TestCase):
    def test_heartbeat_records_local_path_freshness_and_delegated_unknowns(self):
        with TemporaryDirectory() as state_dir, TemporaryDirectory() as local_dir:
            ...
            pack = _lfw_pack_with_local_path(Path(local_dir))
            ...
            self.assertEqual("source_heartbeat.company.lfw", summary["id"])
            self.assertEqual("company.lfw", summary["company_ref"])
            ...
            local = _latest(read_model, "connector.lfw.local")
            ...
            folio = _latest(read_model, "connector.lfw.folio")
            ...
            zulip = _latest(read_model, "connector.lfw.zulip")
            ...
            github = _latest(read_model, "connector.lfw.github_org")
            ...
            transcripts_raw = _latest(read_model, "connector.lfw.transcripts.raw")
    ...
    def test_heartbeat_records_failed_local_path_when_missing(self):
        with TemporaryDirectory() as state_dir:
            ...
            pack = _lfw_pack_with_local_path(missing_path)
            CompanyCapabilityRuntime(stores).seed([pack])
    ...
    def test_cli_runs_source_heartbeat_and_exports_read_model(self):
        with TemporaryDirectory() as state_dir, TemporaryDirectory() as local_dir:
            ...
            CompanyCapabilityRuntime(stores).seed(
                [_lfw_pack_with_local_path(Path(local_dir))]
            )
            output_dir = Path(state_dir) / "source-freshness"
...
def _lfw_pack_with_local_path(path: Path):
    pack = load_json(PACK_DIR / "company-lfw.json")
    source_ref = f"local:{path}"
    for connector in pack["source_connectors"]:
        if connector["id"] in {
            "connector.lfw.local",
            "connector.lfw.transcripts.raw",
        }:
