...
class ActiveHeartbeatTests(unittest.TestCase):
    def test_heartbeat_records_local_path_freshness_and_delegated_unknowns(self):
        with TemporaryDirectory() as state_dir, TemporaryDirectory() as local_dir:
            ...
            pack = _acme_pack_with_local_path(Path(local_dir))
            ...
            self.assertEqual("source_heartbeat.company.acme", summary["id"])
            self.assertEqual("company.acme", summary["company_ref"])
            ...
            local = _latest(read_model, "connector.acme.local")
            ...
            folio = _latest(read_model, "connector.acme.folio")
            ...
            zulip = _latest(read_model, "connector.acme.zulip")
            ...
            github = _latest(read_model, "connector.acme.github_org")
            ...
            transcripts_raw = _latest(read_model, "connector.acme.transcripts.raw")
            ...
            transcripts_processed = _latest(
                read_model,
                "connector.acme.transcripts.processed",
            )
            self.assertEqual("unknown", transcripts_processed["status"])
            self.assertEqual("docs", transcripts_processed["connector_type"])
            self.assertEqual("transcripts:pipeline:acme", transcripts_processed["source_ref"])
            self.assertEqual(
                "delegated_connector",
    ...
    def test_heartbeat_records_failed_local_path_when_missing(self):
        with TemporaryDirectory() as state_dir:
            ...
            pack = _acme_pack_with_local_path(missing_path)
            CompanyCapabilityRuntime(stores).seed([pack])
    ...
    def test_cli_runs_source_heartbeat_and_exports_read_model(self):
        with TemporaryDirectory() as state_dir, TemporaryDirectory() as local_dir:
            ...
            CompanyCapabilityRuntime(stores).seed(
                [_acme_pack_with_local_path(Path(local_dir))]
            )
            output_dir = Path(state_dir) / "source-freshness"
...
def _acme_pack_with_local_path(path: Path):
    pack = load_json(PACK_DIR / "company-acme.json")
    source_ref = f"local:{path}"
    for connector in pack["source_connectors"]:
        if connector["id"] in {
            "connector.acme.local",
            "connector.acme.transcripts.raw",
        }:
