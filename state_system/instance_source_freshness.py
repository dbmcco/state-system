...
def build_instance_source_freshness_read_model(
...
def _validate_freshness_contract(record: JsonObject) -> None:
    """Enforce source-freshness proof semantics before persistence.

    Freshness records are ground-truth/read-model inputs. They must say what
    kind of freshness they prove; connector health, package generation, and
    declared gaps must not be silently promoted to source freshness.
    """
    ...
    if missing:
        raise ValueError(
            "instance source freshness record missing required freshness contract "
